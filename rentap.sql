...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_agreement_duration` (IN `p_unit` VARCHAR(3), IN `p_rentee_id` INT, IN `p_move_in_date` DATE, IN `p_remaining_days` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_maintenance_request` (IN `p_unit` VARCHAR(3), IN `p_rentee_id` INT, IN `p_date` DATE, IN `p_category` VARCHAR(50), IN `p_issue` VARCHAR(100), IN `p_description` TEXT, IN `p_status` ENUM('Pending','Resolved'))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_payment` (IN `p_rentee_id` INT, IN `p_amount` DECIMAL(10,2))   BEGIN
    INSERT INTO Payment_History (rentee_id, date, amount, status)
    VALUES (p_rentee_id, CURDATE(), p_amount, 'Paid');
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `add_payment_record` (IN `p_rentee_id` INT, IN `p_date` DATE, IN `p_amount` DECIMAL(10,2), IN `p_status` ENUM('Paid','Overdue'))   BEGIN
    INSERT INTO Payment_History (rentee_id, date, amount, status)
    VALUES (p_rentee_id, p_date, p_amount, p_status);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `add_rentee` (IN `p_unit` VARCHAR(3), IN `p_first_name` VARCHAR(50), IN `p_last_name` VARCHAR(50), IN `p_facebook_profile` VARCHAR(255), IN `p_email` VARCHAR(100), IN `p_phone_number` VARCHAR(20), IN `p_pin` INT(6))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `delete_agreement_duration` (IN `p_unit` VARCHAR(3), IN `p_rentee_id` INT)   BEGIN
    DELETE FROM Agreement_Duration
    WHERE unit = p_unit AND rentee_id = p_rentee_id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `delete_maintenance_request` (IN `p_request_id` INT)   BEGIN
    DELETE FROM Maintenance_Request
    WHERE request_id = p_request_id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `delete_payment_record` (IN `p_rentee_id` INT, IN `p_date` DATE)   BEGIN
    DELETE FROM Payment_History
    WHERE rentee_id = p_rentee_id AND date = p_date;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `delete_rentee` (IN `p_rentee_id` INT)   BEGIN
    DELETE FROM Rentee
    WHERE rentee_id = p_rentee_id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `edit_payment_record` (IN `p_rentee_id` INT, IN `p_date` DATE, IN `p_amount` DECIMAL(10,2), IN `p_status` ENUM('Paid','Overdue'))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `edit_rentee` (IN `p_rentee_id` INT, IN `p_unit` VARCHAR(3), IN `p_first_name` VARCHAR(50), IN `p_last_name` VARCHAR(50), IN `p_facebook_profile` VARCHAR(255), IN `p_email` VARCHAR(100), IN `p_phone_number` VARCHAR(20))   BEGIN
    UPDATE Rentee
    SET unit = p_unit,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `validate_manager_login` (IN `p_name` VARCHAR(50), IN `p_password` VARCHAR(255))   BEGIN
    SELECT m_name
    FROM Manager
    WHERE m_name = p_name AND password = p_password;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `validate_rentee_login` (IN `p_unit` VARCHAR(3), IN `p_first_name` VARCHAR(50), IN `p_pin` INT(6))   BEGIN
    SELECT rentee_id
    FROM Rentee
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `active_rentees`  AS SELECT `r`.`rentee_id` AS `rentee_id`, `r`.`first_name` AS `first_name`, `r`.`last_name` AS `last_name`, `r`.`unit` AS `unit`, `u`.`status` AS `status` FROM (`rentee` `r` join `unit_status` `u` on(`r`.`unit` = `u`.`unit`)) WHERE `u`.`status` = 'Occupied' ;
...
-- Events
--
CREATE DEFINER=`root`@`localhost` EVENT `update_overdue_status` ON SCHEDULE EVERY 1 DAY STARTS '2025-03-30 14:40:54' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
    UPDATE Payment_History
    SET status = 'Overdue'
    WHERE DATEDIFF(CURDATE(), date) > 7 AND status = 'Paid';
END$$

CREATE DEFINER=`root`@`localhost` EVENT `update_remaining_days` ON SCHEDULE EVERY 1 MINUTE STARTS '2025-03-30 19:30:42' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
    UPDATE Agreement_Duration
    SET remaining_days = GREATEST(60 - DATEDIFF(CURDATE(), move_in_date), 0)
    WHERE move_in_date <= CURDATE();
END$$

CREATE DEFINER=`root`@`localhost` EVENT `create_monthly_pending` ON SCHEDULE EVERY 1 MONTH STARTS '2025-03-30 19:53:52' ON COMPLETION NOT PRESERVE ENABLE DO BEGIN
